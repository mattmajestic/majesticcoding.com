...
async function fetchMatches() {
    try {
        const response = await fetch('/api/epl/schedule');
    ...
    content.innerHTML = `
        <div class="match-container fade-in">
            <div class="team">
                ...
                <div class="team-name">${match.home_team.name.replace(' FC', '').replace(' AFC', '')}</div>
            </div>
            <div class="vs">VS</div>
            <div class="team">
                <img src="${match.away_team.crest}" alt="${match.away_team.name}" class="team-crest">
                <div class="team-name">${match.away_team.name.replace(' FC', '').replace(' AFC', '')}</div>
            </div>
        </div>
