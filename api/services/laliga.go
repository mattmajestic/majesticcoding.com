...
func FetchLaLigaSchedule() ([]models.LaLigaMatch, error) {
	...
	matches := make([]models.LaLigaMatch, len(apiResponse.Matches))
	for i, match := range apiResponse.Matches {
		...
		matches[i] = models.LaLigaMatch{
			ID:       match.ID,
			Date:     date,
			Status:   match.Status,
			Matchday: match.Matchday,
			HomeTeam: models.LaLigaTeam{
				ID:    match.HomeTeam.ID,
				Name:  match.HomeTeam.Name,
				Crest: match.HomeTeam.Crest,
			},
			AwayTeam: models.LaLigaTeam{
				ID:    match.AwayTeam.ID,
				Name:  match.AwayTeam.Name,
				Crest: match.AwayTeam.Crest,
			},
			Score: models.LaLigaScore{
				Winner:   winner,
				Duration: match.Score.Duration,
				FullTime: models.LaLigaResult{
					Home: match.Score.FullTime.Home,
					Away: match.Score.FullTime.Away,
				},
				HalfTime: models.LaLigaResult{
